{{ config (
    alias = target.database + '_pinterest_uk_performance_by_ad_group'
)}}

{%- set date_granularity_list = ['day','week','month','quarter','year'] -%}
{%- set exclude_fields = ['date','last_updated','unique_key'] -%}
{%- set dimensions = ['advertiser_id','campaign_id','ad_group_id'] -%}
{%- set measures = adapter.get_columns_in_relation(ref('pinterest_uk_ad_groups_insights'))
                    |map(attribute="name")
